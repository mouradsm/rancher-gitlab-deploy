...
@click.option(
    ...
    default=None,
    required=True,
    help="The name of the project in Rancher (defaults to the name of the group in GitLab)",
)
@click.option(
    "--namespace",
    ...
    default=None,
    required=True,
    help="The name of the namespace in Rancher (defaults to the name of the group in GitLab)",
)
@click.option(
    "--service",
...
def main(
    ...
    try:
        ...
        projects = r.json()["data"]  ## environments == projects
    ...
    for def_secret in defined_secrets:
        ...
        try:
            r = session.get("%s/projects/%s/secrets?name=%s" % (apiv2, project_id, def_secret["name"]))
            r.raise_for_status()
        except HTTPError:
            bail(
            ...
            if len(result) > 0:
                ...
                bail("Cannot find secret %s in environment %s ?!" % (def_secret["name"], project_id))

    # 2 -> Find the stack in the environment

    ## REFATORAR PARA NOVA ESTRUTURA DE NAMESPACES // STACK == NAMESPACE
    try:
        r = session.get("%s/projects/%s/environments?limit=1000" % (api, project_id))
        r.raise_for_status()
    except HTTPError:
        bail("Unable to fetch a list of stacks in the environment '%s'" % project_name)
    else:
        namespaces = r.json()["data"]

    for n in namespaces:
        if n["name"].lower() == namespace.lower():
            namespace = n
            break
    else:
        if create:
            new_namespace = {"name": namespace.lower()}
            try:
    ...
    try:
        r = session.get("%s/projects/%s/environments/%s/services?limit=1000" % (api, environment_id, namespace["id"]))
        r.raise_for_status()
    except HTTPError:
        ...
        if create:
            ...
            try:
                ...
                if defined_service_links:
                    ...
                    r = session.post(service["actions"]["setservicelinks"],
                                     json={"serviceLinks": defined_service_links})
                    r.raise_for_status()
                    service = r.json()
    ...
    try:
        r = session.post("%s/projects/%s/services/%s/?action=upgrade" % (api, environment_id, service["id"]),
                         json=upgrade)
        r.raise_for_status()
    except HTTPError:
    ...
    if not wait_for_upgrade_to_finish:
        ...
        if not finish_upgrade:
            ...
            try:
                r = session.post(
                    "%s/projects/%s/services/%s/?action=finishupgrade" % (api, environment_id, service["id"]))
                r.raise_for_status()
            except HTTPError:
